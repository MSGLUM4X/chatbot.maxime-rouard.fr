...
import {FormContext} from "@/app/(chatbot)/components-page/form-client";
...
const PromptBar = () => {
    ...
    const {formRef, isPending, state} = context;
            ...
            <div className="mt-2 mb-2 mx-auto p-3">
                <div className="flex items-end gap-2 ">
                    ...
                    <textarea
                        ...
                        placeholder={state.errors?.prompt.length > 0 ? state.errors?.prompt[0] :"Écrivez votre prompt..."}
                        rows={1}
                        className={clsx("flex-1 w-full placeholder-gray-500 bg-white px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-black resize-none min-h-[48px] max-h-32",
                            {"placeholder-red-400 ": state.errors?.prompt.length > 0}
                            )}
